...
public class BeanMaker<T> implements Maker<T>
{
   ...
   private static Object getExistingOrMakeNew(Parameter parameter)
   {
      ...
      Settable settableParameter = SettableParameter.from(parameter);
      return existingValue.isPresent() ? existingValue.get() : ScalarValueMakerFactory.FACTORY.from(settableParameter).value();
   }

   private Object getExistingOrMakeNew(Field field)
   {
      Type fieldType = field.getGenericType();
      Optional<?> existingValue = holder.tryGet(TypeToken.of(fieldType));
      return existingValue.isPresent() ? existingValue.get() : ScalarValueMakerFactory.FACTORY.from(SettableField.from(type, field)).value();
   }
